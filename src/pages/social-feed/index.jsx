      ...
      <div className="sticky top-0 z-50 bg-card border-b border-border shadow-soft">
        <div className="max-w-screen-xl mx-auto px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-xl sm:text-2xl font-heading font-semibold text-foreground truncate">
              Communauté
            </h1>
      ...
      <main className="main-content flex-1">
        <div className="max-w-screen-xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
              ...
              <div className="sticky top-24 space-y-4">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-4 sm:p-6">
                  <div className="text-center mb-4">
                    {userAvatar ? (
                      <img
                        src={userAvatar}
                        alt={userName}
                        className="w-16 h-16 sm:w-20 sm:h-20 rounded-full mx-auto object-cover mb-3"
                      />
                    ) : (
                      <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl sm:text-2xl mx-auto mb-3">
                        {userName?.charAt(0).toUpperCase() || 'U'}
                      </div>
                    )}
                    <h3 className="font-heading font-semibold text-gray-900 text-base sm:text-lg">{userName}</h3>
                    {currentProfile && (
                      <p className="text-xs sm:text-sm text-gray-600">🐕 {currentProfile.name}</p>
                    )}
                  </div>
                  
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className="text-center">
                      <div className="font-bold text-gray-900 text-sm sm:text-base">{userStats.posts}</div>
                      <div className="text-xs text-gray-600">Posts</div>
                    </div>
                    <div className="text-center">
                      <div className="font-bold text-gray-900 text-sm sm:text-base">{userStats.followers}</div>
                      <div className="text-xs text-gray-600">Abonnés</div>
                    </div>
                    <div className="text-center">
                      <div className="font-bold text-gray-900 text-sm sm:text-base">{userStats.following}</div>
                  ...
                  <button
                    ...
                    className="w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 rounded-xl text-xs sm:text-sm font-medium text-gray-700 transition-smooth flex items-center justify-center gap-2"
                  >
            ...
            <div className="flex-1 w-full lg:max-w-2xl lg:mx-auto space-y-4 sm:space-y-6">
              
              <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-4 sm:p-6">
                <button
                  onClick={() => setShowCreatePost(true)}
                  className="w-full flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 bg-gray-100 hover:bg-gray-200 rounded-xl transition-smooth text-left"
                >
                  {userAvatar ? (
                    <img
                      src={userAvatar}
                    ...
                    <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm sm:text-base flex-shrink-0">
                      {userName?.charAt(0).toUpperCase() || 'U'}
                    </div>
                  )}
                  <span className="text-sm sm:text-base text-gray-600">Quoi de neuf avec ton chien ?</span>
                  <Plus size={18} className="sm:w-5 sm:h-5 ml-auto text-blue-500 flex-shrink-0" />
                </button>
              </div>
              
              <div className="flex gap-1 sm:gap-2 bg-white rounded-3xl shadow-sm border border-gray-200 p-1 sm:p-2">
                <button
                  onClick={() => setFeedType('following')}
                  className={`flex-1 flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 sm:py-3 rounded-xl font-medium transition-smooth text-sm sm:text-base ${
                    feedType === 'following'
                ...
                >
                  <UsersIcon size={18} className="sm:w-5 sm:h-5" />
                  <span>Explorer</span>
                </button>
              </div>
              
              <div className="flex gap-1 sm:gap-2 overflow-x-auto pb-1 sm:pb-2 scrollbar-hide -mx-3 sm:mx-0 px-3 sm:px-0">
                {TAGS.map((tag) => (
                  <button
                    key={tag}
                    onClick={() => setSelectedTag(tag)}
                    className={`px-3 sm:px-4 py-2 sm:py-3 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap transition-smooth flex-shrink-0 ${
                      selectedTag === tag
                        ? 'bg-blue-500 text-white'
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-3xl p-6 sm:p-12 text-center border border-gray-200">
                  <p className="text-gray-600 text-sm sm:text-base">Aucun post pour le moment</p>
                </div>
              )}
            </div>
            ...
            <aside className="hidden md:block md:w-72 lg:w-80 flex-shrink-0">
              <div className="sticky top-24 space-y-4">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-4 sm:p-6">
                  <h3 className="font-heading font-semibold text-gray-900 mb-4 flex items-center gap-2 text-base sm:text-lg">
                    <UserPlus size={18} className="sm:w-5 sm:h-5 text-blue-500" />
                    Qui suivre
                  </h3>
                ...
                {tagStats.length > 0 && (
                  <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-4 sm:p-6">
                    <h3 className="font-heading font-semibold text-gray-900 mb-4 flex items-center gap-2 text-base sm:text-lg">
                      <Hash size={18} className="sm:w-5 sm:h-5 text-blue-500" />
                      Tags populaires
                    </h3>
...
const PostCard = ({ post, currentUserId, currentUserAvatar, currentUserName, onUpdate, isTopPost }) => {
  ...
  const cardClasses = isTopPost 
    ? "bg-gradient-to-br from-orange-50 to-yellow-50 border-2 border-orange-200 rounded-3xl shadow-sm p-4 sm:p-6 transition-smooth"
    : "bg-white border border-gray-200 rounded-3xl shadow-sm p-4 sm:p-6 transition-smooth";
  ...
  return (
    <div className={cardClasses}>
      <div className="flex items-start gap-2 sm:gap-3 mb-3 sm:mb-4">
        ...
        >
          {authorAvatar ? (
            <img
              src={authorAvatar}
              alt={authorName}
              className="w-8 h-8 sm:w-12 sm:h-12 rounded-full object-cover cursor-pointer hover:opacity-80 transition-smooth"
            />
          ) : (
            <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm sm:text-base cursor-pointer hover:opacity-80 transition-smooth">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 sm:gap-2">
            <button
              onClick={() => navigate(`/profile/${post.user_id}`)}
              className="font-semibold text-gray-900 hover:underline text-sm sm:text-base truncate"
            >
              {authorName}
            </button>
            {isTopPost && <TrendingUp size={14} className="sm:w-4 sm:h-4 text-orange-500 flex-shrink-0" />}
          </div>
          <span className="text-xs sm:text-sm text-gray-600">{formatDate(post.created_at)}</span>
        </div>
      </div>
      
      {post.title && (
        <h3 className="text-base sm:text-lg font-heading font-semibold text-gray-900 mb-1 sm:mb-2">{post.title}</h3>
      )}
      
      <p className="text-sm sm:text-base text-gray-900 whitespace-pre-wrap leading-relaxed mb-3 sm:mb-4">{post.content}</p>
      ...
      <div className="flex items-center gap-3 sm:gap-6 pt-3 sm:pt-4 border-t border-gray-200">
        <button onClick={handleLike} className="flex items-center gap-1 sm:gap-2">
          <Heart 
            size={20} 
            className={`sm:w-6 sm:h-6 ${hasLiked ? 'text-red-600' : 'text-gray-400'}`}
            fill={hasLiked ? 'currentColor' : 'none'} 
          />
          <span className="text-sm sm:text-base font-medium text-gray-700">{localLikeCount}</span>
        </button>
        
        <button onClick={() => setShowComments(!showComments)} className="flex items-center gap-1 sm:gap-2">
          <MessageCircle size={20} className="sm:w-6 sm:h-6 text-gray-400" />
          <span className="text-sm sm:text-base font-medium text-gray-700">{post.comment_count || 0}</span>
        </button>
        
        <button onClick={() => setShowShareModal(true)} className="flex items-center gap-1 sm:gap-2 ml-auto">
          <Share2 size={20} className="sm:w-6 sm:h-6 text-gray-400" />
        </button>
      </div>
      ...
      {showComments && (
        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200 space-y-3 sm:space-y-4">
          <form onSubmit={handleCommentSubmit} className="flex gap-2 sm:gap-3">
            {currentUserAvatar ? (
              <img src={currentUserAvatar} alt="Avatar" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover flex-shrink-0" />
            ) : (
            ...
            <div className="flex-1">
              <textarea
                ...
                className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 resize-none"
                rows={2}
              />
              <button
                type="submit"
                disabled={!newComment.trim() || submittingComment}
                className="mt-1 sm:mt-2 px-3 sm:px-4 py-2 sm:py-3 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 transition-smooth disabled:bg-gray-300 flex items-center gap-1 sm:gap-2 text-sm sm:text-base"
              >
          ...
          {loadingComments ? (
              ...
              {comments.map((comment) => {
                ...
                return (
                  <div key={comment.id} className="flex gap-2 sm:gap-3">
                    ...
                    >
                      {commentAuthorAvatar ? (
                        <img src={commentAuthorAvatar} alt={commentAuthorName} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover cursor-pointer hover:opacity-80 transition-smooth" />
                      ) : (
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs sm:text-sm cursor-pointer hover:opacity-80 transition-smooth">
                          {commentAuthorName.charAt(0).toUpperCase()}
                        </div>
                      )}
                    </button>
                    <div className="flex-1">
                      <div className="bg-gray-100 rounded-xl px-3 sm:px-4 py-2 sm:py-3">
                        <button
                          onClick={() => navigate(`/profile/${comment.user_id}`)}
                          className="font-semibold text-xs sm:text-sm text-gray-900 mb-1 hover:underline block"
                        >
                          {commentAuthorName}
                        </button>
                        <p className="text-sm sm:text-base text-gray-900">{comment.content}</p>
                      </div>
                      <p className="text-xs sm:text-sm text-gray-600 mt-1 ml-2 sm:ml-3">{formatDate(comment.created_at)}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <p className="text-gray-600 text-center py-4 sm:py-8 text-sm sm:text-base">Aucun commentaire</p>
          )}
        </div>
      )}
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50" onClick={() => setShowShareModal(false)}>
          <div className="bg-white rounded-t-3xl sm:rounded-3xl p-4 sm:p-6 max-w-md w-full space-y-3 sm:space-y-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg sm:text-xl font-heading font-semibold text-gray-900 mb-3 sm:mb-4">Partager</h3>
            <button onClick={copyText} className="w-full flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 sm:py-4 rounded-xl hover:bg-gray-100 text-left">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <span className="font-medium text-gray-900 text-sm sm:text-base">Copier le texte</span>
            </button>
            <button onClick={shareToWhatsApp} className="w-full flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 sm:py-4 rounded-xl hover:bg-gray-100 text-left">
            ...
            <button onClick={copyLink} className="w-full flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 sm:py-4 rounded-xl hover:bg-gray-100 text-left">
              ...
              <span className="font-medium text-gray-900 text-sm sm:text-base">Copier le lien</span>
            </button>
          </div>
        </div>
      )}
      ...
      {showShareToast && (
        <div className="fixed bottom-20 sm:bottom-24 left-1/2 transform -translate-x-1/2 z-50 bg-green-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl shadow-xl text-sm sm:text-base">
          ✅ Copié !
        </div>
      )}
    </div>
